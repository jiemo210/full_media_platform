...
### 12.7 一键成稿任务流（v3.0 M1 已落地）
- 顶部导航「⚡ 成稿任务」+ 热点新闻「一键成稿」入口：一次点击自动完成 **素材收集 → AI 成文 → 风控检查 → 自动修订 → 入库**
- 后台 Worker 顺序执行（DB 状态 + 事件总线 + SSE 实时进度），失败可重试、可取消、服务重启自动续跑
- 风控高风险/中风险时按建议自动修订一轮后入库；支持完成后自动创建发布任务（可选平台）
- 每阶段产物与风控结果入库，详情页展示阶段时间线；与既有 AI 创作/改写入口并存
